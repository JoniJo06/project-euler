...
static int get_item_from_grid(int numbers[GRID_SIZE][GRID_SIZE], int row,
                              int col) {
...
void largest_product_in_a_grid(retTuple *ret) {
  ret->name = "11# largest product in a grid";
  ...
  for (i = 0; i < GRID_SIZE - 3; i++) {
    for (j = 0; j < GRID_SIZE - 3; j++) {
      ...
      int d1 = grid[i][j] * grid[i + 1][j + 1] * grid[i + 2][j + 2] *
               grid[i + 3][j + 3];
      int d2 = grid[i][GRID_SIZE - j - 1] * grid[i + 1][GRID_SIZE - j - 2] *
               grid[i + 2][GRID_SIZE - j - 3] * grid[i + 3][GRID_SIZE - j - 4];
